...
- 文件使用 Worker 根目录下的相对路径，目录路径以 `/` 结尾
- 文件路径遵循 Linux 命名规则；系统会自动移除路径开头的 `/` 和各级名称首尾空格
- Worker 根目录计为第 1 层，服务端默认最多允许 3 层目录，实际限制可由服务端环境变量调整
- 创建嵌套文件时会自动创建缺少的父目录；创建空目录时将 `filename` 以 `/` 结尾
...
- 删除目录会递归删除其中全部文件和子目录，执行前必须向用户明确说明并取得确认
